...
$stmt = $conn->prepare("SELECT " .implode(", ", $sql_field_names). ", voluntary_work_id FROM voluntary_work WHERE user_uuid=?");

$stmt->bind_param("s", $_SESSION["uuid"]);
$stmt->execute();
$stmt->store_result();
$stmt->bind_result( $F_0, $F_1, $F_2, $F_3, $voluntary_work_id);
...
if(isset($_POST["INSERT"])){
    ...
    $response = onSubmit($conn, $query_type, "voluntary_work", $sql_field_names, $object, null);
...
if(isset($_POST["REMOVE"])){
    ...
    $stmt = $conn->prepare("SELECT voluntary_work_id FROM voluntary_work WHERE voluntary_work_id=? AND user_uuid=?");

    $stmt->bind_param("ss", $id, $_SESSION["uuid"]);
    $stmt->execute();
    $stmt->store_result();
    if ($stmt->num_rows > 0) {
        $stmt = $conn->prepare("DELETE FROM voluntary_work WHERE voluntary_work_id=?");
...
<table style='width: fit-content'>
    <tr>
        <th><center>Name and address</center></th>
...
if ($stmt->num_rows <= 0) {
    echo "<h3>You have no voluntary works</h3>";
} else {
    echo "
        <table style='width: fit-content'>
            <tr>
        ...
        echo "
            <tr>
                ...
                <td style='align-items: center;'>
                    <center>
                        <form action='' method='POST'>
                            <button type='submit' name='REMOVE' value='$voluntary_work_id' style='
