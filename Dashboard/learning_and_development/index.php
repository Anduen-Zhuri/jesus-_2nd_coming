...
$fields = array(
    'Learning and Development title',
    'Date from',
    'Date to'
);
...
$stmt = $conn->prepare("SELECT " .implode(", ", $sql_field_names). ", lrd_ID FROM learning_and_development WHERE user_uuid=?");

$stmt->bind_param("s", $_SESSION["uuid"]);
$stmt->execute();
$stmt->store_result();
$stmt->bind_result( $F_0, $F_1, $F_2, $lrd_ID);
...
if(isset($_POST["INSERT"])){
    ...
    $response = onSubmit($conn, $query_type, "learning_and_development", $sql_field_names, $object, null);
...
if(isset($_POST["REMOVE"])){
    ...
    $stmt = $conn->prepare("SELECT lrd_ID FROM learning_and_development WHERE lrd_ID=? AND user_uuid=?");

    $stmt->bind_param("ss", $id, $_SESSION["uuid"]);
    $stmt->execute();
    $stmt->store_result();
    if ($stmt->num_rows > 0) {
        $stmt = $conn->prepare("DELETE FROM learning_and_development WHERE lrd_ID=?");
...
<table style='width: fit-content'>
    <tr>
        <th><center>Learning and Development title</center></th>
        ...
        echo "
            <tr>
                ...
                <td style='align-items: center;'>
                    <center>
                        <form action='' method='POST'>
                            <button type='submit' name='REMOVE' value='$lrd_ID' style='
